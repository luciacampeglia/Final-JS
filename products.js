...
//Buscador
/* const searchingSweater = products.filter (sweater => 
    sweater.name == "Sweater" ||sweater.name == "sweater" ||sweater.name == "sueter")
    let sweaterSearch = document.getElementById('btnAdd');
    sweaterSearch.onclick = searchingSweater */

let filter = document.addEventListener("keyup", e=>{
    if (e.target.matches("#searcher")){
    document.querySelectorAll(".products").forEach(prenda => {
